...
const rewriteBySource = new Map(rewrites.filter((item) => !(item.has?.length)).map((item) => [item.source, item]));
...
const designSystemCanonical = '/work/elearning-design-system';
const observedDesignSystemSlugs = [
  'overview', 'atomic-design', 'asset-register', 'colours', 'core-more-bore',
  'course-structure', 'images-icons', 'storyline', 'typography',
];
for (const slug of observedDesignSystemSlugs) {
  assertRedirect(
    `/design-system/${slug}`,
    designSystemCanonical,
    `observed Design System legacy route /design-system/${slug}`,
  );
}
assertRedirect('/work/isq-elearning-design-system', designSystemCanonical, 'retired ISQ-specific Design System canonical');
...
if (!wordpressQueryRewrite) fail('Inherited WordPress query identities need the conditional /?p= rewrite to /api/wordpress-legacy');
if (!existsSync('api/gone.js')) fail('api/gone.js is missing');
if (!existsSync('api/wordpress-legacy.js')) fail('api/wordpress-legacy.js is missing');

const retiredWordPressResponse = await wordpressLegacyMiddleware(new Request('https://glennhammond.com/?p=156456'));
if (retiredWordPressResponse.status !== 410) fail(`Routing middleware returned ${retiredWordPressResponse.status} for retired /?p=156456, expected 410`);

const preservedWordPressResponse = await wordpressLegacyMiddleware(new Request('https://glennhammond.com/?p=156463'));
if (
...
const unknownWordPressResponse = await wordpressLegacyMiddleware(new Request('https://glennhammond.com/?p=999999'));
if (unknownWordPressResponse.status !== 404) fail(`Routing middleware returned ${unknownWordPressResponse.status} for unknown /?p=999999, expected 404`);
...
for (const entry of liveSitemapMigration) {
  ...
  if (entry.action === 'retain') {
    ...
    if (!routeFileExists(destination)) fail(`${entry.path}: retained destination ${destination} is not statically rendered`);
    const loc = `${SITE}${destination === '/' ? '/' : destination}`;
    if (!sitemap.includes(`<loc>${loc}</loc>`)) fail(`${entry.path}: retained destination ${destination} is not in the canonical sitemap`);
  }
  if (entry.action === 'redirect') assertRedirect(entry.path, entry.destination, entry.path);
...
for (const entry of inheritedPortfolioRedirects) {
  if (entry.launchReady) assertRedirect(entry.source, entry.destination, entry.source);
}

for (const entry of inheritedArticleRedirects) {
  if (!entry.launchReady) continue;
  if (entry.action === 'preserve') {
    if (!routeFileExists(entry.destination)) fail(`${entry.slug}: preserved destination ${entry.destination} is not statically rendered`);
    if (!sitemap.includes(`<loc>${SITE}${entry.destination}</loc>`)) fail(`${entry.slug}: preserved destination ${entry.destination} is not in the canonical sitemap`);
    ...
    const queryRedirect = redirects.find((redirect) =>
      ...
      redirect.has?.some((condition) => condition.type === 'query' && condition.key === 'p' && condition.value === entry.wpId),
    );
    if (!queryRedirect) fail(`/?p=${entry.wpId}: preserved WordPress query redirect to ${entry.destination} is missing`);
  }
  if (entry.action === 'retire') {
...
note(`Live-estate snapshot: ${liveSitemapCapture.urlCount} sitemap URLs captured from ${liveSitemapCapture.sourceProject} on ${liveSitemapCapture.capturedAt}`);
...
note(`Legacy sitemap article source-state: ${legacySitemapArticleSourceCapture.articleCount} entries — ${Object.entries(legacySourceCounts).map(([state, count]) => `${state} ${count}`).join(', ')}`);
note(`Inherited WordPress estate: ${inheritedRedirectCapture.articleCount} article identities / ${inheritedRedirectCapture.articleSourceForms} historical source forms captured from ${inheritedRedirectCapture.sourceProject}`);
note(`Inherited source survival: ${inheritedSourceCapture.articleCount} identities — ${Object.entries(inheritedSourceCounts).map(([state, count]) => `${state} ${count}`).join(', ')}`);
...
const dependencyGaps = Object.entries(migrationDependencies).filter(([, ready]) => !ready).map(([name]) => name);
if (dependencyGaps.length) note(`Migration dependencies still open: ${dependencyGaps.join(', ')}`);
else note('Launch-scale migration dependencies reconciled.');

if (unresolved.length) {
  const message = `${unresolved.length} currently live sitemap URLs are not cutover-ready`;
  if (PUBLISHING) fail(message); else warn(message);
}
