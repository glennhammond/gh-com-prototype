...
export function validateRecordContent(content) {
  ...
  unique(content.projects, 'project');
  unique(content.records, 'record');
  unique(content.artefacts, 'artefact');
  unique(content.evidenceClaims, 'evidence claim');

  const projectIds = new Set(content.projects.map((item) => item.id));
  const recordIds = new Set(content.records.map((item) => item.id));
  const artefactIds = new Set(content.artefacts.map((item) => item.id));
  const claimIds = new Set(content.evidenceClaims.map((item) => item.id));
  const paths = new Set();
  ...
  for (const record of content.records) {
    ...
    const project = content.projects.find((item) => item.id === record.projectId);
  ...
  for (const artefact of content.artefacts) {
    assert(recordIds.has(artefact.recordId), `artefact ${artefact.id} references missing record ${artefact.recordId}`);
    const record = content.records.find((item) => item.id === artefact.recordId);
  ...
  for (const claim of content.evidenceClaims) {
...
export function indexRecordContent(content) {
  ...
  return {
    projectById: Object.fromEntries(content.projects.map((item) => [item.id, item])),
    recordById: Object.fromEntries(content.records.map((item) => [item.id, item])),
    artefactById: Object.fromEntries(content.artefacts.map((item) => [item.id, item])),
    claimById: Object.fromEntries(content.evidenceClaims.map((item) => [item.id, item])),
  };
}
